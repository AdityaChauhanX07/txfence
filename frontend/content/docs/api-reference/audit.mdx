...
Each entry captures: `action`, `policySnapshot` (immutable clone at decision time), `evaluation`, `simulationResult`, `approvalRequest`, `approvalDecision`, and `outcome`.

`AuditOutcome` covers: `success`, `policy_rejected`, `simulation_failed`, `approval_timeout`, `execution_failed`, `dry_run`.
